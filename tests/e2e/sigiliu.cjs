...
(async () => {
  ...
  for (const vp of [{ width: 1180, height: 820 }, { width: 820, height: 1180 }]) {
    ...
    ok(/^1 cu sigiliu$/.test(t1.trim()), `${vp.width}: categoria deschisă → „${t1.trim()}”`);
    ...
    ok(t2.trim() === `1 cu sigiliu: ${gl}`, `${vp.width}: categoria restrânsă → „${t2.trim()}” (litera ${gl})`);
    ...
    ok(/1 cu sigiliu/.test(await p.locator('#add-ner .cat-sigiliu').innerText().catch(() => '')), `${vp.width}: bara rândurilor adăugate → „1 cu sigiliu”`);
    ...
    ok(/2 sigilii aplicate/.test(chips), `${vp.width}: Istoric → „2 sigilii aplicate” (${chips.replace(/\n/g, ' | ')})`);
    ok(/^2 nereguli grave\n2 sigilii aplicate/.test(chips), `${vp.width}: după „2 nereguli grave” (G13 + rândul adăugat grav)`);
    await p.screenshot({ path: `${S}/sigiliu-istoric-${vp.width}.png` });
    await p.goto('http://localhost:8080/#/panou'); await p.waitForTimeout(400);
    ok(/2 sigilii aplicate/.test(await p.locator('#sec-open .item:has-text("Depozit Sigma")').innerText()), `${vp.width}: Panou, Controale neîncheiate → „2 sigilii aplicate”`);
    await p.goto('http://localhost:8080/#/obiective'); await p.waitForTimeout(300);
    ok(/2 sigilii aplicate la ultimul control/.test(await p.locator('.obj-card:has-text("Depozit Sigma")').innerText()), `${vp.width}: Obiective → „2 sigilii aplicate la ultimul control”`);
    ...
    ok(/2 sigilii aplicate/.test(await p.locator('.tl-item .ctl-row').first().innerText()), `${vp.width}: pagina obiectivului → rândul controlului`);
    ...
    await p.goto('http://localhost:8080/#/istoric'); await p.waitForTimeout(300);
    ok(/1 sigiliu aplicat/.test(await p.locator('.ctl-row:has-text("Depozit Sigma") .chips').innerText()), `${vp.width}: rândul G trecut pe Conform → „1 sigiliu aplicat”`);
    await ctx.close();
  }

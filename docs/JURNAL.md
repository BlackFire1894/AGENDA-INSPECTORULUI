...
Ultima actualizare: v1.24.0 — 25.09.2026.
...
| 1.24 | **Aplicația nativă iOS** (`ios/`, XcodeGen): înveliș WKWebView peste GitHub Pages (App-Bound Domains → service worker), widgeturi (ecran principal mic/mediu/mare, ecran blocat cerc/dreptunghi/rând) cu cifrele pe 21 de zile, notificări locale (schimbarea stadiului amenzilor, ultima zi ANAF/ASI/încărcare, activități, backup la 7 zile, sărbători), cifra urgentelor pe iconiță, partajare și tipărire prin iOS; în web: `js/nativ.js` (nu face nimic în browser) |
...
- **Aplicația nativă (v1.24, cod scris, necompilat încă pe Mac):** utilizatorul: MacBook Air M2 (8 GB), Xcode 27, iPad cu iPadOS 27, iPhone 12 Pro Max cu iOS 27, cont Apple gratuit. Identificatori: `ro.cucuta.agenda`, `ro.cucuta.agenda.widget`, grup `group.ro.cucuta.agenda`. Planul inițial: înveliș nativ (WKWebView) peste aceeași aplicație web, încărcată de pe GitHub Pages (App-Bound Domains, pentru service worker) → actualizările din `main` ajung singure; partea nativă: widgeturi (ecran principal + ecran blocat, cu cifrele Panoului și datele termenelor, ca să se schimbe singure la miezul nopții) și notificări locale (termene, activități, backup, sărbători; max. 64 programate). Doar pentru utilizator (colegii rămân pe varianta web). Cont: întâi gratuit (Personal Team: expiră la 7 zile, 3 aplicații, reinstalare din Xcode); Apple Developer Program 99 USD/an dacă se dovedește util. Datele aplicației native sunt separate de cele din Safari → mutare o singură dată prin backup. Nu se face rescriere în Swift (două coduri, divergență).
